import { WorkerDirectory } from "@/components/worker-directory";

export default async function PublicWorkersPage({
  searchParams,
}: {
  searchParams: Promise<{ trade?: string; state?: string; q?: string }>;
}) {
  const filters = await searchParams;

  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold">Browse tradespeople</h1>
        <p className="text-muted-foreground">
          Verified plumbers, electricians, carpenters and labourers across
          Australia. Sign in as a company to message someone directly.
        </p>
      </div>
      <WorkerDirectory filters={filters} variant="public" />
    </div>
  );
}
